...
### 拠点（Pi）再起動時の挙動 — どちらも自動復帰する

「Tunnel は片方の Pi が落ちると通信が止まる」のは**再起動中の一時的な話**で、復帰後はどちらの方式でも自動で繋ぎ直される。

```
【Tunnel】
東京Pi再起動中   → 金沢のLEDは消灯（データが来ない）
東京Pi起動完了   → Tunnelが自動復活（systemd）
                → 金沢側Pythonが自動再接続（retryロジック必須）
                → 金沢のLEDが自動で再点灯 ✓

【Durable Objects】
東京Pi再起動中   → 金沢のLEDは消灯（同じ）
東京Pi起動完了   → WorkerのURLは変わらないので即再接続
                → 金沢のLEDが自動で再点灯 ✓
```

違いは**再接続の確実性**：

- Tunnel: 受信側に「繋がらなければ数秒後に再試行」という retry ロジックを自分で書く必要がある（`useWebSocket.ts` の `setTimeout(connect, 2000)` と同じ仕組みを Python 側にも実装）
- DO: 繋ぎ先の Worker URL が常に生きているので、デバイスが起動した瞬間に繋がる。実装済みの `hub.py`（3秒間隔リトライ）と ESP32 ファーム（`setReconnectInterval(3000)` + ハートビート）がすでにこの前提で書かれている

### 構築・運用の手間の比較

| | Tunnel | Durable Objects |
|---|---|---|
| 各Piでやること | cloudflared インストール + tunnel設定（各拠点30分程度） | WiFi と WebSocket 接続先URLの設定のみ |
| Cloudflare側でやること | ほぼなし | Worker のデプロイ（既存 colorHub.ts で済み） |
| 設定ファイル | 各Piに config.yml | wrangler.toml（既存） |

### 使い分けの結論

- **プロトタイプ・自宅検証**: Tunnel（無料）で十分
- **展示本番**: DO の方が安定（ハブが常時稼働、どの拠点が落ちても他が巻き添えにならない）
- 現実的な進め方: まず Tunnel で動作確認 → 本番前に DO へ移行。ただし本リポジトリはすでに DO（ColorHub）で動いているため、**そのまま DO を使うのが最短**
...
| # | 内容 | 対象 | 規模 |
|---|---|---|---|
| 1 | ✅済 palette JSON に `source` タグ追加（環境変数 `LOCATION`） | `analyzer/hub.py` | 数行 |
| 2 | ✅済 `LISTEN_SOURCE` フィルタ追加 | `firmware/colorvision_led.ino` | 数行 |
| 2.5 | ✅済 fps間引き（`HUB_FORWARD_FPS`）・時間帯制御（`HUB_ACTIVE_HOURS`、未設定なら常時）・ローカルブロードキャスト（Tunnelモード対応） | `analyzer/hub.py` `main.py` | 済 |
